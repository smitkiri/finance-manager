...
def test_encode_then_decode_roundtrips_user_and_household():
    token = encode_access_token(user_id="u-1", household_id="h-1")
    claims = decode_access_token(token)
    assert claims["sub"] == "u-1"
    assert claims["household_id"] == "h-1"
