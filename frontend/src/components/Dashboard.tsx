...
export const Dashboard: React.FC<DashboardProps> = React.memo(
  ({ expenses, categories, selectedUserId, users, onViewDetails, isLoading, statsFromApi }) => {
      ...
      return categories
        .map((category) => {
          const categoryExpenses = filteredExpenses.filter(
            (expense) => expense.type === 'expense' && expense.category === category
          );
    ...
    const incomeCategoryBreakdown = useMemo(() => {
      ...
      return categories
        .map((category) => {
          const categoryIncome = filteredExpenses.filter(
            (expense) => expense.type === 'income' && expense.category === category
          );
    ...
    const topExpenses = useMemo(() => {
      if (statsFromApi?.topExpenses?.length)
        return statsFromApi.topExpenses.slice(0, topTransactionCount);
    ...
    const topIncome = useMemo(() => {
      if (statsFromApi?.topIncome?.length)
        return statsFromApi.topIncome.slice(0, topTransactionCount);
      return filteredExpenses
    ...
    function prepareCategoryLineData(expenses: Expense[], categoriesToShow: string[]) {
      const monthlyMap = new Map<
        string,
        { month: string; displayMonth: string; [key: string]: any }
      >();
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center mb-2">
          <span className="text-sm text-gray-500 dark:text-gray-400 font-medium">
            Stats for:{' '}
            <span className="font-semibold text-gray-700 dark:text-white">{userLabel}</span>
          </span>
        </div>
        {/* Statistics Cards */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 p-6">
            ...
            <Chart data={categoryLineData} type="line" title="" categoryMode={true} />
          ...
          <Chart data={donutData} type="donut" title="Savings vs Expenses" />
        </div>
